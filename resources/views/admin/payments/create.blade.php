    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.payments.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\Payment::PURPOSE_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('purpose') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="purpose_{{ $key }}" name="purpose" value="{{ $key }}" {{ old('purpose', '') === (string) $key ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('donor') ? 'is-invalid' : '' }}" type="text" name="donor" id="donor" value="{{ old('donor', '') }}">
                @if($errors->has('donor'))
                    <div class="invalid-feedback">
                        {{ $errors->first('donor') }}
                    </div>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\Payment::PAYMENT_METHOD_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('payment_method') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="payment_method_{{ $key }}" name="payment_method" value="{{ $key }}" {{ old('payment_method', '') === (string) $key ? 'checked' : '' }}>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('show_my_name') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="show_my_name" id="show_my_name" value="1" {{ old('show_my_name', 0) == 1 ? 'checked' : '' }}>
