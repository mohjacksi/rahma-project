...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.donation-categories.update", [$donationCategory->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="name">{{ trans('cruds.donationCategory.fields.name') }}</label>
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $donationCategory->name) }}" required>
                @if($errors->has('name'))
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('special') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="special" id="special" value="1" {{ $donationCategory->special || old('special', 0) === 1 ? 'checked' : '' }}>
