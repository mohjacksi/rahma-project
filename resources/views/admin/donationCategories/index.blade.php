...
@can('donation_category_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.donation-categories.create') }}">
                        ...
                        <tr data-entry-id="{{ $donationCategory->id }}">
                            ...
                            <td>
                                @can('donation_category_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.donation-categories.show', $donationCategory->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('donation_category_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.donation-categories.edit', $donationCategory->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('donation_category_delete')
                                    <form action="{{ route('admin.donation-categories.destroy', $donationCategory->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('donation_category_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.donation-categories.massDestroy') }}",
